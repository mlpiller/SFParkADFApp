...
import model.bulkeditgroups.views.MeterOpScheduleRoVORowImpl;
...
/**
 * Description:
 * This class controls behavior of the selectMeterOpSchedulesPage.jspx
 *
 * Change History:
 ...
 */
public class SelectMeterOpSchedules {
    private static ADFLogger adfLogger = ADFLogger.createADFLogger(SelectMeterOpSchedules.class);
    ...
    private RichTable meterOpSchedulesTable;
    private RichQuery meterOpScheduleQuery;

    public SelectMeterOpSchedules() {
    }
    ...
    public void cancelParkingSpaceSelectionsButtonHandler(ActionEvent actionEvent) {
        ...
        // for these Meter OP Schedules
        BulkEditUtils.resetQueryPanelValues(this.getMeterOpScheduleQuery());
    ...
    public void saveParkingSpaces(String typeOfSave){
        ...
        ParkingSpaceGroupsUpdtVOImpl parkingSpacesGroupsVO = groupsAM.getParkingSpaceGroupsUpdtVO1();
        ...
        CollectionModel meterOpSchedModel = (CollectionModel)meterOpSchedulesTable.getValue();
        JUCtrlHierBinding meterOpSchedBinding = (JUCtrlHierBinding)meterOpSchedModel.getWrappedData();
        DCIteratorBinding meterOpSchedDCIteratorBinding = meterOpSchedBinding.getDCIteratorBinding();
        RowSetIterator meterOpSchedRSIterator = meterOpSchedDCIteratorBinding.getRowSetIterator();
        adfLogger.info("\tDEBUG:  RSIterator START Row Count: " + meterOpSchedRSIterator.getRowCount());
        meterOpSchedRSIterator.reset(); // set currency to beginning of the table

        if(meterOpSchedRSIterator.getRowCount() > 0 || meterOpSchedRSIterator.hasNext()){
            getAnotherRow = true;
        }
        
        while(getAnotherRow){
            MeterOpScheduleRoVORowImpl meterOpSchedRow = (MeterOpScheduleRoVORowImpl)meterOpSchedRSIterator.getCurrentRow();
            adfLogger.info("\tDEBUG:  Parking Space ID: " + meterOpSchedRow.getParkingSpaceId());

            if(meterOpSchedRow.getSelectedMeterOpScheduleRow() != null){
                rowIsSelectedForAddToGroup = meterOpSchedRow.getSelectedMeterOpScheduleRow();
            } else {
            ...
            if(rowIsSelectedForAddToGroup){
                ...
                parkingSpaceGroupsRow.setParkingSpaceId(meterOpSchedRow.getParkingSpaceId());
            ...
            if(meterOpSchedRSIterator.hasNext()){
                meterOpSchedRSIterator.next();
            } else {
                getAnotherRow = false;
            }
    ...
    public void setMeterOpSchedulesTable(RichTable meterOpSchedulesTable) {
        this.meterOpSchedulesTable = meterOpSchedulesTable;
    }

    public RichTable getMeterOpSchedulesTable() {
        return meterOpSchedulesTable;
    }
    ...
    public void onQueryListenerHandler(QueryEvent queryEvent) {
        BulkEditUtils.queryListenerHandler(queryEvent, meterOpSchedulesTable);
    } // onQueryListenerHandler


    public void setMeterOpScheduleQuery(RichQuery meterOpScheduleQuery) {
        this.meterOpScheduleQuery = meterOpScheduleQuery;
    }

    public RichQuery getMeterOpScheduleQuery() {
        return meterOpScheduleQuery;
    }

} // SelectMeterOpSchedules
